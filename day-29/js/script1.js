...
function getCartItems() {
  ...
  cartItems.forEach(item => {
    container.innerHTML += `
      <div class="col-12 card m-5" style="width: 18rem;">
  ...
  const totalValue = Math.floor(cartItems.reduce((acc, item) => acc + item.price * item.quantity, 0));

  const totalAmountEle = document.getElementById("yourtotal");
  totalAmountEle.innerText = `Total Amount : ₹ ${totalValue}`;

  const disscountEle = document.getElementById("discount");
  const totalEle = document.getElementById("total");

  if (totalValue > 500) {
    const discountValue = Math.floor(totalValue * 0.10);
    const finalTotal = totalValue - discountValue;

    disscountEle.innerText = `Discount (10%) : ₹ ${discountValue}`;
    totalEle.innerText = `Final Total : ₹ ${finalTotal}`;
  } else {
    disscountEle.innerText = `No discount applied`;
    totalEle.innerText = `Final Total : ₹ ${totalValue}`;
  }
}
