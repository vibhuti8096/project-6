...
products.forEach(prct => {
let prd = JSON.parse(localStorage.getItem('product')) || [];

  str += `
    <div class="col-3">
      <div class="card mt-5 p-3" style="width: 20rem;">
        <a href="card.html"><img src="${prct.thumbnail}" alt=""></a>
        <h5 class="card-title text-secondary">${prct.title || 'Card title'}</h5>
        <h6 class="card-subtitle text-body-secondary">${prct.subtitle || ''}</h6>
        <div class="d-flex justify-content-between">
        <span class="item-price">${prct.price}Rs.</span>
        <span class="item-dis text-end">${prct.discountPercentage}% off</span>
        </div>
        <div class="d-flex">
         <span class="item-rating bg-success rounded p-2">${prct.rating} <i class="fa-solid fa-star"></i></span>
        </div>
        <div class="d-flex">
          <button  class="m-2 p-2 rounded" onclick='addToCart(${prct.id})'><a href="" class="card-link text-decoration-none text-primary">Add Card</a></button>
          <button class="m-2 p-2 rounded">
            <a href="" class="card-link text-decoration-none text-primary m-3">
              <span><i class="fa-solid fa-angles-right"></i></span>Buy Now
            </a>
          </button>
        </div>
      </div>
...
function renderBadge() {
  let bgItem = document.getElementById("bedgeItems");
